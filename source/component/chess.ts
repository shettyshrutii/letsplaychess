...
function getUpMoves(type: string, position: string): string[] {
    let upMoves: string[] = [];
    ...
    if (type === "queen") {
        for (let i = row; i <= UPPER_ROW_LIMIT; i++) {
            ...
            upMoves = newPos != position.toUpperCase() ? upMoves.concat(column + i) : upMoves;
        }
    }
    if (type === "king") {
        upMoves = row < UPPER_ROW_LIMIT ? upMoves.concat(column + (row + 1)) : upMoves;
    }
    if (type === "pawn") {
        upMoves = row < UPPER_ROW_LIMIT ? upMoves.concat(column + (row + 1)) : upMoves;
    }
...
function getDownMoves(type: string, position: string): string[] {
    let downMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        for (let i = row; i >= LOWER_ROW_LIMIT; i--) {
            const newPos = column + i
            downMoves = newPos != position.toUpperCase() ? downMoves.concat(column + i) : downMoves;
        }
    }
    if (type === "king") {
        downMoves = row > LOWER_ROW_LIMIT ? downMoves.concat(column + (row - 1)) : downMoves;
    }
    return downMoves;
}

function getLeftMoves(type: string, position: string): string[] {
    let leftMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        switch (column) {
            ...
            case 'F':
                ...
                break;
            case 'G':
                leftMoves = ['A' + row, 'B' + row, 'C' + row, 'D' + row, 'E' + row, 'F' + row];
                break;
            case 'H':
                leftMoves = ['A' + row, 'B' + row, 'C' + row, 'D' + row, 'E' + row, 'F' + row, 'G' + row];
                break;
            default:
                leftMoves = [];
                break;
    ...
    if (type === "king") {
        ...
        leftMoves = previousAlphabet ? leftMoves.concat(previousAlphabet + row) : leftMoves;
    }
    return leftMoves;
}

function getRightMoves(type: string, position: string): string[] {
    let rightMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        switch (column) {
            case 'A':
                rightMoves = ['B' + row, 'C' + row, 'D' + row, 'E' + row, 'F' + row, 'G' + row, 'H' + row];
                break;
            case 'B':
                rightMoves = ['C' + row, 'D' + row, 'E' + row, 'F' + row, 'G' + row, 'H' + row];
                break;
            case 'C':
    ...
    if (type === "king") {
        ...
        rightMoves = nextAlphabet ? rightMoves.concat(nextAlphabet + row) : rightMoves;
    }
    return rightMoves;
}

function getDiagonalLeftUpMoves(type: string, position: string): string[] {
    let diagonalLeftUpMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        ...
        for (let i = row + 1; i <= UPPER_ROW_LIMIT; i++) {
            ...
            diagonalLeftUpMoves = newPos != position.toUpperCase() ? diagonalLeftUpMoves.concat(newColumn + i) : diagonalLeftUpMoves;
        }
    }
    if (type === "king") {
        const previousAlphabet = getPreviousAlphabet(column);
        diagonalLeftUpMoves = row < UPPER_ROW_LIMIT && previousAlphabet 
...
function getDiagonalLeftDownMoves(type: string, position: string): string[] {
    let diagonalLeftDownMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        ...
        for (let i = row - 1; i >= LOWER_ROW_LIMIT; i--) {
            ...
            const newPos = previousAlphabet + i
            newColumn = previousAlphabet;
            diagonalLeftDownMoves = newPos != position.toUpperCase() ? diagonalLeftDownMoves.concat(newColumn + i) : diagonalLeftDownMoves;
        }
    }
    if (type === "king") {
        const previousAlphabet = getPreviousAlphabet(column);
        diagonalLeftDownMoves = row > LOWER_ROW_LIMIT && previousAlphabet 
...
function getDiagonalRightUpMoves(type: string, position: string): string[] {
    let diagonalRightUpMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        ...
        for (let i = row + 1; i <= UPPER_ROW_LIMIT; i++) {
            ...
            diagonalRightUpMoves = newPos != position.toUpperCase() ? diagonalRightUpMoves.concat(newColumn + i) : diagonalRightUpMoves;
        }
    }
    if (type === "king") {
        const nextAlphabet = getNextAlphabet(column);
        diagonalRightUpMoves = row < UPPER_ROW_LIMIT && nextAlphabet 
...
function getDiagonalRightDownMoves(type: string, position: string): string[] {
    let diagonalRightDownMoves: string[] = [];
    const column = position.substring(0, 1).toUpperCase();
    const row = Number(position.substring(1, 2));
    if (type === "queen") {
        ...
        for (let i = row - 1; i >= LOWER_ROW_LIMIT; i--) {
            ...
            const newPos = previousAlphabet + i
            newColumn = previousAlphabet;
            diagonalRightDownMoves = newPos != position.toUpperCase() ? diagonalRightDownMoves.concat(newColumn + i) : diagonalRightDownMoves;
        }
    }
    if (type === "king") {
        const nextAlphabet = getNextAlphabet(column);
        diagonalRightDownMoves = row > LOWER_ROW_LIMIT && nextAlphabet 
...
function getPreviousAlphabet(alphabet: string): string | undefined {
...
function getNextAlphabet(alphabet: string): string | undefined {
    let nextAlphabet;
    switch (alphabet) {
        case 'A':
            nextAlphabet = 'B';
